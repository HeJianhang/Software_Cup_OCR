...
tf.app.flags.DEFINE_integer('charset_size', 3820, "Choose the first `charset_size` character to conduct our experiment.")
...
tf.app.flags.DEFINE_integer('batch_size', 100, 'Validation batch size')
tf.app.flags.DEFINE_string('mode', 'freezing', 'Running mode. One of {"train", "valid", "test"}')
...
def build_graph():
    ...
    with tf.control_dependencies([tf.group(*update_ops)]):
        train_op = tf.train.AdamOptimizer(learning_rate=0.1).minimize(loss, global_step=global_step)
...
def main(_):
    ...
    if FLAGS.mode == "train":
        ...
        label_dict = get_label_dict()
        name_list = get_file_list('D:\Code\Python\CPS-OCR-Engine-master\CPS-OCR-Engine-master\ocr\\tmp')
        #binary_pic(name_list)
        #tmp_name_list = get_file_list('../data/tmp')
        # 将待预测的图片名字列表送入predict()进行预测，得到预测的结果及其index
